...
def login(request):
    if request.method == "POST":
        ...
        if password != "council":
            return render(request,'webpages/login.html', {"error": "Wrong Password"})
        if next == "create":
            webpage_name = request.POST.get('webpage_name')
            if webpage_name == "":
                return render(request,'webpages/login.html', {"error": "Enter Valid Website Name"})
            ...
            if curr_webpage == None:
                return render(request,'webpages/login.html', {"error": "Select Valid Website Name"})
